...
- database mysql: per poter usare il progetto creare un database mysql con il nome **finacial_estimated**, usando la ddl presente nella directory [ddl](script-ufficiali/.docker/ddl/ddl.sql); per le credenziali (da modificare) vedere il file [connectionFe.py](script-ufficiali/.docker/connectionFe.py)
