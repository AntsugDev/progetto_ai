...
//echo "sono connesso ...";
    ...
    array_walk($query, function ($item, $key) use (&$newData) {
        ...
        $punteggio = punteggio(floatval($item["diff_reddito"]), $sostenibilita,
            ...
            intval($item["nr_figli"]), $cofficienteK, $riskIndex
        );
        $newData[] = [
            "id" => $item["cliente"],
            "neopatentati" => intval($item["neo_patentato"]) === 0 ? 'SI' : 'NO',
            "nrFigli" => intval($item["nr_figli"]),
...
function punteggio(float $reddito, float $sotenibilita, float $anticipo, bool $formula, bool $neopatentati, int $nrFigli, array $cofficienteK, int &$riskIndex = 0): array|string|int
{
    ...
    if ($sotenibilita >= 0.35 || $cofficienteK[0] >= 1.70) {
        ...
        $riskIndex = $sotenibilita <= 0.20 ? 1 : ($cofficienteK[0] >= 1.40 && ($sotenibilita >= 0.30 && $sotenibilita >= 0.34) ? 4 : 3);
        if ($neopatentati) {
            $riskIndex++;
            return "Finanziamento(" . $cofficienteK[1] . ")";
        }
        if ($nrFigli >= 2) $riskIndex++;
        if ($reddito <= 2500) {
...
{
    try {
        $handle = fopen('../file/data_model_php.csv', 'w+');
        $row = "ID,NEOPATENTATI,NRFIGLI,REDDITONETTO,FORMULAACQUISTO,COSTOAUTO,IMPORTOFIN,TAN,NRRATE,RATAMENS,SOTENIBILITA,COEFF,PUNTEGGIO,INDEX_RISCHIO\n";
        array_walk($array, function ($item) use (&$row) {
            $row .= $item['id'].','.$item['neopatentati'].','.$item['nrFigli'].','.
                $item['reddito_netto'].','.$item['formula'].','.$item['costo_auto'].','.
                $item['importo_fin'].','.$item['tan'].','.$item['nr_rate'].','.
                $item['rata_mensile'].','.$item['sostenibilita'][1].','.$item['coefficiente_k'][0].','.
                $item['punteggio'].','.$item['indice_di_rischio']."\n";
        });
        fwrite($handle, $row);
        fclose($handle);
