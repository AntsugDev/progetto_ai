...
def compute_risk_index(row):
    ...
    # base index (valutazione "pura")
    if s >= 0.35 or (pd.notna(K) and K > 1.7):
        idx = 5  # non concedibile
    elif 0.30 <= s < 0.35 or (pd.notna(K) and 1.40 < K <= 1.70):
        idx = 4  # rischio alto
    elif 0.21 <= s <= 0.29:
        idx = 3  # rischio medio (zona grigia)
    elif 0.15 < s < 0.21:
        idx = 2  # rischio basso
    else:
        idx = 1  # rischio molto basso (ottima sostenibilità)
