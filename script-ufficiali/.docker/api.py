...
#
        modelRevision = ModelRevision(sostenibilita[0],X)
        revision, lastId = modelRevision.migliore_scelta()
        ...
        decisionAI = calcoli.decisionAI(sostenibilita[0])    
        ...
        cc = c.cursor()
        with cc as cursor:
            cursor.execute(INSERT_PREVISIONING, ( float(request.reddito), float(request.request), float(importo_rata[0]), float(sostenibilita[0]), decisionAI, lastId if lastId else None))
            c.commit()
