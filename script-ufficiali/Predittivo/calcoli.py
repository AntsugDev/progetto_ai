...
if __name__ == "__main__":
    ...
    # Test calcoli con auto usata (scenario revisione)
    dati_test = {
        ...
        'nr_rate': 48,
        'tan': 5.0,
        'anticipo_perc': 0,  # Nessun anticipo per auto usata in prima istanza
        'nr_figli': 1,
        'neo_patentato': 'No'
    }
    
    risultati = calc.calcola_tutti_i_valori(dati_test)
    print("🧮 RISULTATI CALCOLI:")
    for key, value in risultati.items():
        if key != 'simulazione':
            print(f"   {key}: {value}")
    
    # Mostra simulazione se presente
    if risultati.get('simulazione'):
        print("\n🔍 SIMULAZIONE:")
        sim = risultati['simulazione']
        if sim['simulazione_anticipo_solo_auto_usata']:
            print("   📊 Simulazione Anticipo:")
